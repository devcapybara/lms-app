...
// Debug endpoint to promote user to mentor/admin (temporary)
router.post('/debug/promote/:userId', auth, async (req, res) => {
  try {
    const { userId } = req.params;
    const { role } = req.body; // 'mentor' or 'admin'
    
    if (!['mentor', 'admin'].includes(role)) {
      return res.status(400).json({ message: 'Role harus mentor atau admin' });
    }
    ...
    res.json({
      message: `User ${user.name} berhasil dipromosikan dari ${oldRole} ke ${role}`,
      user: user.getPublicProfile()
    });
  } catch (error) {
...
// Debug endpoint to check current user and all users (temporary)
router.get('/debug/users', auth, async (req, res) => {
  try {
    ...
    res.json({
      currentUser: currentUser.getPublicProfile(),
